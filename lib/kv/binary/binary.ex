defmodule KV.Binary do
  ...
  def from_binary( value ) when is_bitstring( value ) do
    ...
    bin_list = String.split( value, ~r{} )             # split the string into characters
                |> filter( fn x -> x != "" end )                  # drop any chars that are empty strings (String.split terminates the list with an empty string for some reason)
                |> map( fn x ->String.to_integer(x) end )        # convert each one into an int; this returns tuples of {i, ""}
